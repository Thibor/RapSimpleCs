	...
	class CRapSimpleCs
	{
		...
		static void Main()
		{
			...
			void Initialize()
			{
				...
				for (int x = 0; x < 8; x++)
					for (int y = 0; y < 8; y++)
					{
						...
						CBitBoard.Add(ref bbProtector[iw, 0], x - 1, y - 1);
						CBitBoard.Add(ref bbProtector[iw, 0], x - 1, y - 2);
						//bbProtector[ib, 0] = CBitBoard.FlipY(bbProtector[iw, 1]);
						for (int n = y - 1; n >= 0; n--)
						{
						...
						//bbOutpost[ib, 0] = CBitBoard.FlipY(bbOutpost[iw, 1]);
						for (int n = 1; n < 8; n++)
						{
						...
						//bbPassed[ib, 0] = CBitBoard.FlipY(bbPassed[iw, 1]);
						...
						CBitBoard.Add(ref bbSupported[iw, 0], x + 1, y - 1);
						//bbSupported[ib, 0] = CBitBoard.FlipY(bbSupported[iw, 1]);
					}
			}
			...
			int GetScore(List<int> mu, int ply, int depth, int alpha, int beta)
			{
				...
				if ((g_phase > 24) && (!IsAttacked(!whiteTurn, kingPos)) && (ply > 2) &&  (depth - ply > 2))
				{
					whiteTurn = !whiteTurn;
					osScore = Quiesce(1, depth, -beta, -beta + 1, lastInsufficient, lastScore);
					whiteTurn = !whiteTurn;
					if ((osScore >= beta) && (osScore > -0xf000) && (osScore < 0xf000))
						return beta;
				}
			...
			void Search(int depth, int time, int nodes)
			{
				...
				do
				{
					...
					depthCur++;
				} while (((depth == 0) || (depth > depthCur - 1)) && (os > -0xf000) && (os < 0xf000) && !g_stop && (mu.Count > 1));
